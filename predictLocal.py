...
from google_drive_downloader import GoogleDriveDownloader as gdd # download models from google drive
### IMPORTS ###

### DOWNLOAD MODELS FROM GOOGLE DRIVE###
modelPaths = []
downloadNeeded = input("Have you downloaded the models(yes/no)?")
if downloadNeeded.lower() == 'yes':
    downloadPath = input("Please enter the path you have downloaded the models to:")
    modelPaths = [downloadPath+'/modelColor.pth', downloadPath+'/modelComposition.pth', downloadPath+'/modelDoF.pth',
    downloadPath+'/modelPalette.pth', downloadPath+'/modelType.pth']
elif downloadNeeded.lower() == 'no':
    downloadPath = input("Each model is approximately 95MB in size (5x95=475MB total). Please enter the path you want to download the Models to:")
    modelIds = ['14djl57tT21qqi7C2bfxVd19sgb3R6g6T', '1dtXUTLR3f0iF-Ez9ElVt0ppyOEh8iRDJ', '1iojCerM7bvChFHwn5MtiG_jFAYBYkGWw',
    '11j1-JgfpM-Zxs10W319DwAzskV7mUVsC', '1hsMUj77niXLu6b1-812R_vrTTx9b5XU3']
    modelNames = ['Color', 'Composition', 'DoF', 'Palette', 'Type']
    i = 0
    for modelId in modelIds:
        print("Downloading Model for the", modelNames[i], "task on path:", downloadPath)
        destinationPath = downloadPath+"/model"+modelNames[i]+".pth"
        gdd.download_file_from_google_drive(file_id=modelId, dest_path=destinationPath, unzip=True)
        modelPaths.append(destinationPath)
        i = i + 1
else:
    print("Acceptable answers 'yes' or 'no'. Terminating..")
    exit()
### DOWNLOAD MODELS FROM GOOGLE DRIVE###
...
checkpointColor = torch.load(modelPaths[0])
...
checkpointDoF = torch.load(modelPaths[2])
...
checkpointPalette = torch.load(modelPaths[3])
...
checkpointComposition = torch.load(modelPaths[1])
...
checkpointType = torch.load(modelPaths[4])
